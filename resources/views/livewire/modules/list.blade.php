<div class="card">
    ...
    <div class="card-footer">
        <div class="row g-1 mb-md-1">
            ...
            <div class="col-md-4">
                <a wire:ignore class="btn btn-sm btn-info" href="{{url('/modules/create')}}">
                    <i class="fa fa-plus" class="me-50"></i> Añadir Modulo
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td data-bs-toggle="modal" data-bs-target="#moduleTabModal" wire:click="general({{$row->id}})">{{ $row->formative_module }}</td>
                    <td data-bs-toggle="modal" data-bs-target="#moduleTabModal" wire:click="general({{$row->id}})">{{ $row->name }}</td>
                    <td data-bs-toggle="modal" data-bs-target="#moduleTabModal" wire:click="general({{$row->id}})">{{ $row->total_hours }}</td>
                    <td data-bs-toggle="modal" data-bs-target="#moduleTabModal" wire:click="general({{$row->id}})"><span class="badge rounded-pill badge-light-{{$row->active == 0 ?'danger' : 'success'}} me-1">{{$row->active == 0 ? 'Inactivo' : 'Activo'}}</span></td>
                    <td>
                        <div class="dropdown">
                            ...
                            <!--<a data-bs-toggle="modal" data-bs-target="#updateModal" class="dropdown-item" wire:click="edit({{$row->id}})"><i class="fa-regular fa-pen-to-square"></i> Editar </a>-->
                                <a href="{{url('/modules/edit/'.$row->id)}}" class="dropdown-item edit"><i class="fa-regular fa-pen-to-square"></i> Editar </a>
                                @if ($row->activo == 1)
