                ...
                <div wire:ignore>
                    ...
                    <select wire:model.lazy="training_unit_id" class="form-control select2" id="training_unit_id">
                        ...
                        @foreach($training_units as $training_unit)
                            <option value="{{$training_unit['id']}}">{{$training_unit['formative_unit']}} {{$training_unit['name']}}</option>
                        @endforeach
                    </select>
                </div>
                @error('training_unit_id') <div class="invalid-feedback">Tipo de curso es requerido</div> @enderror
            </div>
            <div class="col-12 mb-2">
                @foreach($modules_training_units as $training_unit)
                    <div class="" style="display: flex; background: white; margin: 5px; padding: 5px; justify-content: space-between;">
                        <div style="margin-top: 5px;">
                            {{$training_unit['formative_unit']}} - {{$training_unit['name']}}
                        </div>
                        <div>
                            <div class="btn btn-danger btn-sm unregister" wire:click="unregister({{$training_unit->id}})"><i class="fas fa-minus"></i></div>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <script>
            ...
            document.addEventListener('livewire:load', function() {
                ...
                @this.set(this.id, $(this).val())
                    id = $(this).val();
                    Livewire.emit('addTrainingUnit', id)
                })
            })
