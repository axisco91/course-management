<div class="card-body">
    <form>
        <div class="row">
            ...
            <div class="col-md-4 col-12">
                <div wire:ignore>
                    <label class="form-label" for="tracing_id">Seguimiento</label>
                    <select wire:model.lazy="tracing_id" class="form-select select2" id="tracing_id" placeholder="Seguimiento" disabled>
                        <option value="">Selección un seguimiento</option>
                        @foreach($tracings as $tracing)
                            <option value="{{$tracing['id']}}">{{$tracing['course_name']}} {{$tracing['student_name']}} {{$tracing['student_surname']}}</option>
                        @endforeach
                    </select>
                </div>
                @error('tracing_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
