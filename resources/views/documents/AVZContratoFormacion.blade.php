...
<p class="no-line-break"> En lo no previsto en este contrato, se estará a la legislación vigente que resulte de aplicación y particularmente a lo dispuesto en el artículo 11 del Estatuto de los Trabajadores. Asimismo le será de aplicación lo dispuesto en el Convenio Colectivo de
    <span class="dots"> {{ $trainingContract->applicableAgreement ? $trainingContract->applicableAgreement->name : ($trainingContract->company ? ($trainingContract->company->agreement) : '') }} </span></p>
<br>
...
<div class="mt-4 borde-redondeado">
    ...
    <p class="text-left">
    <p class="text-center">
        <span class="aditional-fondo-azul">
            @if(!empty($trainingContract->document_additional_clause))
                {{-- Display only the clause text --}}
                {{ $trainingContract->document_additional_clause }}
            @else
