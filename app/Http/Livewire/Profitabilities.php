...
class Profitabilities extends Component
{
    ...
    public $courses, $companies, $students, $tab = 'info', $course_statuses, $group, $company_name, $beginning, $end, $students_list, $search_student_name, $search_student_surname, $registrations;
    ...
    public function render()
    {
        ...
        if ($this->selected_id){
            $profitabilities = Profitability::find($this->selected_id);
            $this->registrations = $profitabilities->registrations()->get()->pluck('student_id')->toArray();

            $search_student_name = '%'.$this->search_student_name.'%';
            $search_student_surname = '%'.$this->search_student_surname.'%';
            $this->students_list = Student::whereIn('id', $this->registrations)
