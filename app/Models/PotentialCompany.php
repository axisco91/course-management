...
class PotentialCompany extends Model
{
    ...
    protected $fillable = ['name','nif','company_type_id','company_activity_id','email','telephone','legal_representative','dni_legal_representative','quote','cnae_id','average_template','iban','sepa','b2b','address','post_code','population_id','province_id','population','advisor_id', 'collaborator_id'];
    ...
    public static function getPotentialCompanies($keyWord, $search_name, $search_nif, $search_type_id, $search_activity_id, $search_province_id){
        $companies = PotentialCompany::select('potential_companies.*', 'company_types.name as type',
            ...
            DB::raw("'potential' as type_company"))
    ...
    public static function updatePotentialCompany($id, $data){
        $potential = PotentialCompany::find($id);
        $potential->update([
            'name' => $data['name'],
            'nif' => $data['nif'],
            'company_type_id' => $data['company_type_id'],
