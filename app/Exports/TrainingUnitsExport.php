...
class TrainingUnitsExport implements FromCollection, WithHeadings
{
    use Exportable;

    public function __construct($formative_units, $name, $professional_family_id,  $professional_area_id, $modality_id, $provider_id, $active){
        $this->formative_units = $formative_units;
        $this->name = $name;
    ...
    {
        $training_units = TrainingUnit::select('formative_unit', 'training_actions.name', 'action_types.name as action_name',
        ...
        'course_origins.name as course_origin',
            \DB::raw('(CASE
        ...
        if ($this->professional_family_id){
            $training_units = $training_units->Where('training_actions.professional_family_id', $this->professional_family_id);
        }
        if ($this->professional_area_id){
            $training_units = $training_units->Where('training_actions.professional_area_id', $this->professional_area_id);
        }
        if ($this->modality_id){
            $training_units = $training_units->Where('training_actions.modality_id', $this->modality_id);
        }
        if ($this->provider_id){
            $training_units = $training_units->Where('training_actions.provider_id', $this->provider_id);
        }

        $training_units = $training_units->get();

        return collect($training_units);
    }
